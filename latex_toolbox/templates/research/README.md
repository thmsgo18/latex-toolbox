...
This project was generated from the `research` template of LaTeX Toolbox.

It is intended for research-style reports, technical papers, and academic writing in English.
...
1. `latexmk`, `lualatex`, or `bibtex` is not installed
...
git init
git add .
git commit -m "Initial research draft"
```
...
git commit -m "Initial research draft"
git branch -M main
gh repo create your-paper-repo-name --private --source=. --remote=origin --push
```

Replace `your-paper-repo-name` with the repository name you want on GitHub.
